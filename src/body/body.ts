...
export class Body {
    ...
    constructor(opt: BodyOpt, type: number) {
        ...
        this.sleepCounter = 0;
        this.mask = 1;
        this.positionImpulse = new Vector(0, 0);
        this.prevPositionImpulse = new Vector(0, 0);
        this.parts = [];
        this.bound = null;
     ...
     * @param fn 
     */
    setRender(fn: (body: Body, dt: number, timeScale: number, timeStamp: number) => void) {
    ...
    applyImpulse(impulse: Vector, offset: Vector) {
        ...
        this.angularVelocity += this.invInertia * offset.cor(impulse);
    }
    ...
    applyForce(force: Vector, offset?: Vector) {
        ...
        if(offset) {
            this.torque += offset.cor(this.force);
        }
    }
     ...
     * 更新物理
     * 使用改进半隐式欧拉积分
     * 参考：https://www.richardlord.net/blog/presentations/physics-for-flash-games.html
     * @param dt 步长
     * @param timeScale 时间缩放系数
     */
    updatePhysics(dt: number, timeScale: number) {
        let deltaTime = dt * timeScale,
            // 计算摩擦力
            airFriction = 1 - this.airFriction * timeScale * timeScale,
            acceleration = this.force.scl(this.invMass);
         
        let newVelocity = this.velocity.scl(airFriction).add(acceleration.scl(deltaTime)),
            newAngularVelocity = (this.angularVelocity * airFriction) + (this.torque * this.invInertia) * deltaTime;
        
        this.velocity.x = (this.velocity.x + newVelocity.x) * 0.5;
        this.velocity.y = (this.velocity.y + newVelocity.y) * 0.5;
        this.angularVelocity = (this.angularVelocity + newAngularVelocity) * 0.5;
        ...
        //位移刚体
        this.translate(this.velocity.scl(deltaTime));
        // 旋转刚体
        if(this.angularVelocity !== 0) {
            this.rotate(this.angularVelocity * deltaTime, this.position);
        }
    }
} 