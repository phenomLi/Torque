...
class Creator {
    ...
    init() {
        this.zr.on('click', (e) => {
            ...
            if(this.selectedShape) {
                this.selectedBody = this.selectedShape.t;
            }
        });
    }
    ...
    remove(body) {
        ...
        if(body.parent) {
            ...
            parent.parts.map(item => {
                this.zr.remove(item.data);
            });
        }
    }
    ...
    circle(x, y, r, opt) {
        ...
        return this.apply(circle, circleShape);
    }
    ...
    rect(x, y, w, h, opt) {
        ...
        return this.apply(rect, rectShape);
    }
    ...
    isogon(x, y, r, n, opt) {
        ...
        return this.apply(isogon, isogonShape);
    }
    ...
    polygon(x, y, points, opt) {
        ...
        return this.apply(poly, polyShape);
    }

    composite(bodies, opt) {
        for(let i = 0; i < bodies.length; i++) {
            this.t.remove(bodies[i]);
        }

        let composite = Torque.body.Composite(bodies, opt);

        this.t.append(composite);
