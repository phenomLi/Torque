...
class Renderer {
    ...
    createIsogon(x, y, radius, edges, options) {
        ...
        isogon.position.y = y;  
        isogon.moveTo(0, 0);
        isogon.lineTo(vertices[0][0], vertices[0][1]);  
    ...
    createAnchor(x, y, body, options) {
        let anchor = Torque.joint.anchor(Torque.vector(x, y), body),
            anchorBody = anchor.anchorBody,
            circle = new PIXI.Graphics(),
            anchorRadius = 3;

        circle.lineStyle(1, options.stroke, 1);
        circle.position.x = anchorBody.position.x;
        circle.position.y = anchorBody.position.y;    
        circle.drawCircle(0, 0, anchorRadius);

        anchor.setRender(function() {
            circle.position.x = anchorBody.position.x;
            circle.position.y = anchorBody.position.y;
        });

        setTimeout(() => {
            anchor.discard && circle.clear();
        }, 0);
     ...
     * @param {*} options 
     */
    createJoint(b1, b2, options) {
        let stiffness = options.stiffness === undefined? 1: options.stiffness,
            joint = Torque.joint.create(b1, b2, stiffness),
            line = new PIXI.Graphics();

        joint.setRender(function() {
            line.clear();
            line.lineStyle(1, options.stroke, 1);
            line.moveTo(b1.position.x, b1.position.y);
            line.lineTo(b2.position.x, b2.position.y);
        });

        return this.render(joint, line);
    }
